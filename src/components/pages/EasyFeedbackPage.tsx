...
export default function EasyFeedbackPage() {
  const navigate = useNavigate();
  const easyStage = useAssessmentStore((state) => state.easyStage);
      ...
      <div className="pt-32 pb-24">
        <div className="max-w-[100rem] mx-auto px-8">
          <motion.div
            ...
            className="mb-12"
          >
            <h1 className="font-heading text-5xl text-foreground mb-4">Easy Stage Feedback</h1>
            <p className="font-paragraph text-lg text-muted-gray-foreground">
              Review your performance and learn from the model answers
            </p>
            <div className="w-32 h-1 bg-gradient-to-r from-primary to-secondary mt-6"></div>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/30 rounded-2xl p-8 mb-12"
          >
            <div className="flex items-center justify-between flex-wrap gap-6">
              ...
              <div className="flex items-center gap-4">
                <div className={`font-heading text-6xl ${getRatingColor(easyStage.rating)}`}>
                  {easyStage.rating}
                </div>
                <div className="flex flex-col gap-1">
                  {[...Array(10)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-3 w-3 ${
                        i < easyStage.rating ? 'text-electric-teal fill-electric-teal' : 'text-muted-gray'
                      }`}
                    />
                  ))}
                </div>
          ...
          <div className="space-y-12">
            {easyStage.cases.map((caseItem, caseIndex) => (
              <motion.div
                key={caseIndex}
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.3 + caseIndex * 0.1 }}
                className="bg-gradient-to-br from-secondary/5 to-primary/5 border border-secondary/20 rounded-2xl p-8"
              >
          ...
          >
            <Button
              ...
              className="bg-primary hover:bg-primary/90 text-primary-foreground font-heading text-lg px-12 py-6 rounded-lg transition-all duration-300 hover:scale-105"
            >
              Continue to Medium Stage <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
          </motion.div>
        </div>
      </div>
