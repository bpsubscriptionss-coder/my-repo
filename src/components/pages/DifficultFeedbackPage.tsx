...
export default function DifficultFeedbackPage() {
  const navigate = useNavigate();
  const difficultStage = useAssessmentStore((state) => state.difficultStage);

  if (!difficultStage) {
    navigate('/assessment/difficult');
    return null;
  }
      ...
      <div className="pt-32 pb-24">
        <div className="max-w-[100rem] mx-auto px-8">
          <motion.div
            ...
            className="mb-12"
          >
            <h1 className="font-heading text-5xl text-foreground mb-4">Difficult Stage Feedback</h1>
            <p className="font-paragraph text-lg text-muted-gray-foreground">
              Review your performance and learn from the model answers
            </p>
            <div className="w-32 h-1 bg-gradient-to-r from-electric-teal to-neon-magenta mt-6"></div>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-gradient-to-br from-electric-teal/10 to-neon-magenta/10 border border-electric-teal/30 rounded-2xl p-8 mb-12"
          >
            <div className="flex items-center justify-between flex-wrap gap-6">
              ...
              <div className="flex items-center gap-4">
                <div className={`font-heading text-6xl ${getRatingColor(difficultStage.rating)}`}>
                  {difficultStage.rating}
                </div>
                <div className="flex flex-col gap-1">
                  {[...Array(10)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-3 w-3 ${
                        i < difficultStage.rating ? 'text-electric-teal fill-electric-teal' : 'text-muted-gray'
                      }`}
                    />
                  ))}
                </div>
          ...
          <div className="space-y-12">
            {difficultStage.cases.map((caseItem, caseIndex) => (
              <motion.div
                key={caseIndex}
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.3 + caseIndex * 0.05 }}
                className="bg-gradient-to-br from-neon-magenta/5 to-electric-teal/5 border border-neon-magenta/20 rounded-2xl p-8"
              >
                <div className="flex items-center gap-3 mb-6">
                  <div className="bg-electric-teal/20 px-4 py-2 rounded-lg">
                    <span className="font-heading text-lg text-electric-teal">Case {caseIndex + 1}</span>
                  </div>
          ...
          >
            <Button
              onClick={() => navigate('/assessment/final-report')}
              className="bg-electric-teal hover:bg-electric-teal/90 text-electric-teal-foreground font-heading text-lg px-12 py-6 rounded-lg transition-all duration-300 hover:scale-105"
            >
              View Final Report <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
          </motion.div>
        </div>
      </div>
